...
void response_proceed(h2o_generator_t *generator, h2o_req_t *req) {
    ...
    custom_generator->req_handler(custom_generator->req, custom_generator->res, custom_generator->data);
}
...
void stream_response(bool (*req_handler)(http_req* req, http_res* res, void* data),
    ...
    if(!response.generator) {
        h2o_custom_generator_t* custom_generator = new h2o_custom_generator_t {
            h2o_generator_t {response_proceed, response_stop}, req_handler, &request, &response, data
        };
